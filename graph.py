...
# from static.py.portfolio_stats import Portfolio_Stats
from .portfolio_stats import Portfolio_Stats
import plotly
...
class Security_Portfolio_data():
    ...
    def load_content(self):
        ...
        df.loc[:, 'date'] = df['date'].str.split(' ', n = 4, expand = False).str[1:4]
        df.loc[:, 'date'] = pd.to_datetime(df['date'].str.join('-'))
    ...
    def load_bad_data_content(self, start, end):
        ...
        frame.loc[:, 'weekday'] = pd.to_datetime(frame['date']).dt.dayofweek
    ...
    def portfolio_rand_user_weights(self, weights):
        ...
        stats_init = Portfolio_Stats(returns_daily.copy(), (self.parse_date_content('SPY')['close'] * 10).pct_change())
        beta, alpha, r_2 = stats_init.regression_stats(lst_weights)
        VaR = stats_init.VaR(lst_weights)
        ...
        for ticker, weight in weights.items():
            if weight != '':
                portfolio[ticker + ' Weight'] = float(weight)
        ...
        column_order = (['Portfolio Return', 'Portfolio Deviation', 'Sharpe Ratio', 'Beta', 'Alpha', 'R^2', '90% Confidence Level VaR',
                            '95% Confidence Level VaR', '97.5% Confidence Level VaR', '99% Confidence Level VaR',] + [stock for stock in selected])
    ...
    def portfolio_rand_rand_weights(self, weights, num_portfolios):
        ...
        returns_daily = built_frame.pct_change()
        returns_annual = returns_daily.mean()
        returns_daily_copy = returns_daily.copy()
        ...
        # Initialization of Portfolio statistics
        stats_init = Portfolio_Stats(returns_daily_copy, (self.parse_date_content('SPY')['close'] * 10).pct_change())
...
class Build_graph():
    ...
    def regression_graph(self):
        data = Security_Portfolio_data(self.ticker, (self.dates, self.date_type)).parse_date_content()
        data.loc[:, 'Percent Change'] = data.loc[:, 'close'].pct_change()
        # Use SPY as proxy for SPX
        spx_data = Security_Portfolio_data('SPY', (self.dates, self.date_type)).parse_date_content()
        data.loc[:, 'SPX Pct Change'] = (spx_data.loc[:, 'close'] * 10).pct_change()
    ...
    def portfolio_graph(self, weights, num_portfolios):
        try:
            user_data, _ = Security_Portfolio_data('', (self.dates, self.date_type)).portfolio_rand_user_weights(weights)
            rand_data = Security_Portfolio_data('', (self.dates, self.date_type)).portfolio_rand_rand_weights(_, num_portfolios)
        except:
            rand_data = Security_Portfolio_data('', (self.dates, self.date_type)).portfolio_rand_rand_weights(weights, num_portfolios)
            ...
            if ticker + ' Weight' not in list(rand_data):
                try:
                    if ticker + ' Weight' not in list(user_data) or val == '':
                        user_data[ticker + ' Weight'] = str(0)
                        rand_data[ticker + ' Weight'] = str(0)
                except:
                    rand_data[ticker + ' Weight'] = str(0)
        # --------------------------------------------------------

        rand_text = ['Portfolio: ' + str(x + 1) + '<br>' +
                'Return: ' + rand_data.loc[x, 'Portfolio Return'] + '<br>' +
                'Sigma: ' + rand_data.loc[x, 'Portfolio Deviation'] + '<br>' +
                'Sharpe Ratio: ' + rand_data.loc[x, 'Sharpe Ratio'] + '<br>' +
                'Beta: ' + rand_data.loc[x, 'Beta'] + '<br>' +
                'Alpha: ' + rand_data.loc[x, 'Alpha'] + '<br>' +
                'R^2: ' + rand_data.loc[x, 'R^2'] + '<br>' +
                'AAPL Weight: ' + rand_data.loc[x, 'AAPL Weight'] + '<br>' +
                'AMZN Weight: ' + rand_data.loc[x, 'AMZN Weight'] + '<br>' +
                'FB Weight: ' + rand_data.loc[x, 'FB Weight'] + '<br>' +
                'GOOGL Weight: ' + rand_data.loc[x, 'GOOGL Weight'] + '<br>' +
                'MSFT Weight: ' + rand_data.loc[x, 'MSFT Weight'] + '<br>' +
                'TSLA Weight: ' + rand_data.loc[x, 'TSLA Weight'] + '<br>' +
                'UAA Weight: ' + rand_data.loc[x, 'UAA Weight']
                 for x in range(len(rand_data))
                 ]
        rand_plot = go.Scatter(
                x=rand_data['Portfolio Deviation'].str.strip('%').astype(float) / 100,
                y=rand_data['Portfolio Return'].str.strip('%').astype(float) / 100,
                mode='markers',
                text=rand_text,
                hoverinfo='text',
                marker=dict(color=rand_data['Sharpe Ratio'].astype(float),
        ...
        try:
            ...
            user_plot = go.Scatter(
                x=user_data['Portfolio Deviation'].str.strip('%') / 100,
                y=user_data['Portfolio Return'].str.strip('%') / 100,
                mode='markers',
                text=user_text,
                hoverinfo='text',
        ...
        if user_data.empty:
            # print(rand_data)
            return graph, dict(rand_ports=rand_data)
        return graph, dict(rand_ports=rand_data, user_port=user_data)
